    ...
    return html.Div([
                ...
                dcc.RadioItems(
                    ...
                    options=[
                        ...
                        {'label': 'Период', 'value': 'period', "disabled": True},
                    ],
                    value='month'
...
def date_pick(value):
    ...
    if value == "year":
        return html.Div([
                    dcc.Dropdown(
                        id='year-dropdown',
                        options=[{'label': str(year), 'value': year} for year in range(2016, current_year+1)],
    ...
    return html.Div([
                ...
                dcc.Checklist(
                    ...
                    options=[
                        ...
                        {'label': 'Газовый конденсат', "value" : 'condensate_gas', "disabled": True},
                        {'label': 'Нефть', "value" : 'oil', "disabled": True},
                        {'label': 'Газопродукты', "value" : 'product_gas', "disabled": True},
                    ],
                    value=['nat_gas'],
                ),
    ...
    return html.Div([
                ...
                dcc.RadioItems(
                    ...
                    options=[
                        ...
                        {'label': 'млн.фут', 'value': 'mil_pounds', "disabled": True},
                        {'label': 'тыс.т', 'value': 'tho_tons', "disabled": True},
                        {'label': 'тыс.барр.', 'value': 'tho_bars', "disabled": True},
                        {'label': 'млн.тут', 'value': 'mil_tut', "disabled": True},
                    ],
                    value='mil_m3'
                    # labelStyle={'display': 'block'}  # Отображаем лейблы по одному
                ),
...
from xlsx_parser import create_fig_with_wells, create_fig_plus
#просто тест
# @callback(
...
#     #month
#     #year
#     #в id передать id graph
#     #вернуть просто график -> fig
#     # fig = create_fig_with_wells(year, month)
#     # return fig это только для одного 
#     #таким образом нужно добавить ещё один output
...
def try_to_change_figures(year, month):
    fig1 = create_fig_with_wells(month, year)
    fig2 = create_fig_plus(year)
    return fig1, fig2
