...
from db_main import insert_data
...
########################    
#словарь для скважин
    # total_wells = [11, 13, 15]
    # list_of_assets = get_asset_guid()
    # for item in list_of_assets:
    #     for num in total_wells:
    #         for i in range(num):
    #             random_uuid = str(uuid.uuid4())
    #             query = "INSERT INTO wells (well_guid, well_name, asset_guid) VALUES (%s, %s, %s)"
    #             data = (random_uuid, f"{'Wl_' + str(i)}", item)
    #             insert_data(query, data)
###########################
    
    #daily assets_activity
    total_wells = [11, 13, 15]
    random_uuid = str(uuid.uuid4())
    ...
    #     for asset_guid in list_of_assets:
    #         query = "INSERT INTO about_assets (about_assets_guid, c_date, period_type_guid, asset_guid, active_wells, info_type_guid, total_wells) VALUES (%s, %s, %s, %s, %s, %s, %s)"
    #         data = (random_uuid, single_date, period_type, asset_guid, active_wells, info_type_guid, t_wells)
    #         insert_data(query, data)
